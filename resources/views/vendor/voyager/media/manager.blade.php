            ...
            <li v-for="file in getSelectedFiles()" class="dd-item" :data-url="file">
                <div class="file_link selected" aria-hidden="true" data-toggle="tooltip" data-placement="auto" :title="file">
                    <div class="link_icon">
                        ...
                        <template v-else-if="fileIs(selected_file, 'pdf')">
                            <i class="icon voyager-file-text"></i>
                        </template>
                        <template v-else-if="fileIs(selected_file, 'doc') || fileIs(selected_file, 'docx')">
                            <i class="icon voyager-file-text"></i>
                        </template>
                        <template v-else-if="fileIs(selected_file, 'xls') || fileIs(selected_file, 'xlsx')">
                            <i class="icon voyager-file-text"></i>
                        </template>
                    ...
                    <div v-else-if="selected_files.length == 1" class="right_details">
                        <div class="detail_img">
                            ...
                            <div v-else-if="fileIs(selected_file, 'pdf')">
                                <iframe :src="selected_file.path" width="100%" height="500px"></iframe>
                            </div>
                            <div v-else-if="fileIs(selected_file, 'doc') || fileIs(selected_file, 'docx')">
                                <iframe :src="'https://docs.google.com/viewer?url=' + selected_file.path + '&embedded=true'" width="100%" height="500px"></iframe>
                            </div>
                            <div v-else-if="fileIs(selected_file, 'xls') || fileIs(selected_file, 'xlsx')">
                                <iframe :src="'https://docs.google.com/viewer?url=' + selected_file.path + '&embedded=true'" width="100%" height="500px"></iframe>
                            </div>
            ...
            fileIs: function(file, type) {
                if (typeof file === 'string') {
                    if (type == 'image') {
                        ...
                        return this.endsWithAny(['mp4', 'avi', 'wmv', 'mov', 'mpeg'], file.toLowerCase());
                    } else if (type == 'audio') {
                        return this.endsWithAny(['ogg', 'mp3', 'wav'], file.toLowerCase());
                    } else if (type == 'zip') {
                        return this.endsWithAny(['zip', 'rar', '7z'], file.toLowerCase());
                    } else if (type == 'folder') {
