    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="form-group col-md-3">
                <form method="GET" action="{{ route('coordenadas.show') }}">
                    <br>
                    <select class="form-control select2" name="cobrador_id" id="cobrador_id" onchange="this.form.submit()"
                        required>
                        <option value="" selected="selected">Seleccionar Cobrador</option>
                        @foreach ($cobradores as $cobrador)
                            <option value="{{ base64_encode($cobrador->id) }}"
                                {{ ($cobradorId ?? '') == base64_encode($cobrador->id) ? 'selected' : '' }}>
                                {{ $cobrador->name }}
                            </option>
                        @endforeach
                    </select>
...
@section('content')
    ...
    <script>
        ...
        var icons = {
            redIcon: L.icon({
                iconUrl: '{{ asset('images/marker/red_marker.png') }}',
                iconSize: [35, 41],
                iconAnchor: [12, 41],
                popupAnchor: [1, -34]
            }),
            purpleIcon: L.icon({
                iconUrl: '{{ asset('images/marker/purple_marker.png') }}',
                iconSize: [35, 41],
                iconAnchor: [12, 41],
                popupAnchor: [1, -34]
            }),
            greenIcon: L.icon({
                iconUrl: '{{ asset('images/marker/green_marker.png') }}',
                iconSize: [35, 41],
                iconAnchor: [12, 41],
                popupAnchor: [1, -34]
            }),
            yellowIcon: L.icon({
                iconUrl: '{{ asset('images/marker/yellow_marker.png') }}',
                iconSize: [35, 41],
                iconAnchor: [12, 41],
                popupAnchor: [1, -34]
            }),
            calipsoIcon: L.icon({
                iconUrl: '{{ asset('images/marker/calipso_marker.png') }}',
                iconSize: [35, 41],
                iconAnchor: [12, 41],
                popupAnchor: [1, -34]
            }),
                ...
                if (location.persona) {
                    ...
                    switch (location.cobrador_id) {
                        case 2:
                            ...
                            break;
                        case 3:
                            markerIcon = icons.purpleIcon;
                            break;
                        case 4:
                            markerIcon = icons.greenIcon;
                            break;
                        case 5:
                            markerIcon = icons.yellowIcon;
                            break;
                        case 6:
                            markerIcon = icons.calipsoIcon;
                            break;
                        default:
                            markerIcon = icons.defaultIcon;
                    }
